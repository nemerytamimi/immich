...
export async function up(db: Kysely<any>): Promise<void> {
  ...
  await sql`INSERT INTO "migration_overrides" ("name", "value") VALUES ('index_asset_offloadedAt_idx', '{"type":"index","name":"asset_offloadedAt_idx","sql":"CREATE INDEX \\"asset_offloadedAt_idx\\" ON \\"asset\\" (\\"offloadedAt\\") WHERE (\\"offloadedAt\\" IS NOT NULL);"}'::jsonb);`.execute(
    db,
  );
}
...
export async function down(db: Kysely<any>): Promise<void> {
  ...
  await sql`DELETE FROM "migration_overrides" WHERE "name" = 'index_asset_offloadedAt_idx';`.execute(db);
}
